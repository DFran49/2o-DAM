...
class Actores
{
	...
	function insertar()
	{
		...
		$stmt->bind_param("siiss", $this->nombre, $this->edad, $this->activo,strip_tags(""),strip_tags(""));
		if ($stmt->execute()) {
	...
	function actualizar()
	{
		...
		$stmt = $this->conn->prepare("
		    UPDATE " . $this->tabla . " 
			SET nombre= ?, edad = ?, activo = ? WHERE id = ?");

		$this->nombre = strip_tags($this->nombre);
		$this->edad = strip_tags($this->edad);
		$this->activo = strip_tags($this->activo);
		$this->id = strip_tags($this->id);
		$stmt->bind_param("siii", $this->nombre, $this->edad, $this->activo, $this->id);
